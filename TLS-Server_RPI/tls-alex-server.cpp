...
#define AlexPvt   "alex.key"
#define ALexCert  "alex.crt"
#define CACert   "signing.pem"
#define CommonName   "laptop.epp.com"
...
void handleColour(TPacket *packet)
{
	...
	resp[1] = RESP_COLOUR;
	resp[2] = (char) packet->params[10];
	sendNetworkData(resp, sizeof(resp));
	
}

void handleUltra(TPacket *packet,int type)
{
		char resp[3];
		...
		if(type == 0){
			resp[1] = RESP_ULTRA;
			resp[2] = (char) packet->params[11];
			printf("%d",packet->params[11]);
		}else{
			resp[1] = RESP_ULTRA_BACK;
			resp[2] = (char) packet->params[11];
...
char resp[2];
	switch(packet->command)
	{
		...
		case RESP_ULTRA:
			handleUltra(packet,0);
		break;
		case RESP_ULTRA_BACK:
			handleUltra(packet,1);
		break;
...
void handleResp(char * resp){
			resp[0] = NET_ERROR_PACKET;
			resp[1] = RESP_SYS_UP;
			printf("All connections successfully established");
			sendNetworkData(resp, sizeof(resp));
}
...
int main()
{
	...
	//Keep looping until arduino succesfully establishes connection with RPi.
	while(system_up == 0);
	
	createServer(AlexPvt,ALexCert,5000,&worker,CACert,CommonName,1);
